...
export interface RtpengineNode {
  id: string;
  host: string;
  port: number;
...
/** Phase 17 — RTPengine cluster node registry and selection (config unchanged). */
@Injectable()
export class RtpengineNodeRegistryService implements OnModuleInit {
  private readonly logger = new Logger(RtpengineNodeRegistryService.name);
  private nodes: RtpengineNode[] = [];
  ...
  onModuleInit(): void {
    ...
    const intervalMs = Number(this.config.get('RTPENGINE_HEALTH_INTERVAL_MS') ?? '30000');
    if (intervalMs > 0) {
  ...
  listNodes(): RtpengineNode[] {
    return [...this.nodes];
  }

  selectNode(): RtpengineNode | null {
  ...
  async checkAll(): Promise<RtpengineNode[]> {
  ...
  private loadNodes(): RtpengineNode[] {
    const nodesRaw = this.config.get<string>('RTPENGINE_NODES');
    const defaultPort = Number(this.config.get('RTPENGINE_NG_PORT') ?? '2223');
    const defaultHost = this.config.get<string>('RTPENGINE_HOST') ?? 'localhost';
  ...
  private async checkNode(node: RtpengineNode): Promise<void> {
    ...
    if (prev === 'up' && node.status === 'down') {
      this.logger.warn(JSON.stringify({ event: 'ha.rtpengine.node.down', node: node.id }));
      this.events.emit(HA_EVENTS.RTPENGINE_NODE_DOWN, { nodeId: node.id });
    } else if (prev !== 'up' && node.status === 'up') {
      this.events.emit(HA_EVENTS.RTPENGINE_NODE_UP, { nodeId: node.id });
    }
  }
